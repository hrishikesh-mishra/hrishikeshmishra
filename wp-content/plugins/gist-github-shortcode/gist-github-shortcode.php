...
class Gist_Github_Shortcode {
	...
	function shortcode( $atts ) {
		...
		if ($this->isGoogleBot())
			return $this->dumpGistContent($id, $file);
		else
			return $this -> generateGistScriptTag($id, $file);
//		$gist = sprintf(
...
//		return $gist;
	...
	function generateGistScriptTag($id, $file){
		 return  sprintf(
			 '<script src="https://gist.github.com/%s.js%s"></script>',
			 esc_attr( $id ),
			 $file ? '?file=' . esc_attr( $file ) : ''
	...
	function dumpGistContent($id, $file){
		$content = "";
		try {
			$url = sprintf(
				'https://gist.githubusercontent.com/hrishikesh-kumar/%s/raw/%s',
				esc_attr( $id ),
				$file ? '?file=' . esc_attr( $file ) : '');

			$content = "<pre>" . file_get_contents($url) . "</pre>";
		}catch(Exception $e){
			// Do nothing
		}
		return $content;
	...
	function isGoogleBot(){
		...
		if(!empty($_SERVER['HTTP_USER_AGENT']) &&
			stristr(strtolower($_SERVER['HTTP_USER_AGENT']), "googlebot"))
			 return true;
		else
			return false;
